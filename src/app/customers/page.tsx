...
export default function CustomersPage() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-24 w-full max-w-full overflow-x-hidden">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-border pb-6">
        ...
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          <DialogTrigger asChild><Button className="w-full md:w-auto bg-primary font-bold shadow-lg"><Plus className="h-4 w-4 mr-2" /> {t('newProfile')}</Button></DialogTrigger>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /><Input placeholder={t('search')} className="pl-10 h-12" value={search} onChange={e => setSearch(e.target.value)} /></div>
      ...
      <div className="bg-card border rounded-xl overflow-hidden shadow-xl w-full">
                ...
                return (
                  <TableRow key={customer.id} className={cn(!customer.status || customer.status === 'active' ? "" : "opacity-60")}>
                    <TableCell className="pl-6">
                      <Link href={`/customers/${customer.id}`} className="font-bold hover:text-primary transition-colors block truncate max-w-[200px]">{customer.name}</Link>
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground truncate max-w-[200px]"><MapPin className="h-3 w-3 shrink-0" /> {customer.address} • {customer.phone}</div>
                    </TableCell>
